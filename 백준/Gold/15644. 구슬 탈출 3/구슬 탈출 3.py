...
while q:
    rx, ry, bx, by, order, cnt, o = q.popleft()
    for dir in range(4):
        new_rx, new_ry, new_bx, new_by, no = solve(dir, rx, ry, bx, by)
        if dir == order:
            continue
        ...
        if not visited[new_rx][new_ry][new_bx][new_by]:
            q.append((new_rx, new_ry, new_bx, new_by, dir, cnt + 1, o + no))
